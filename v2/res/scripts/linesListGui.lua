...
function linesListGui.showOrCreateUi()
    ...
    if uiElems.window ~= nil then
        print("-- init window --")
        return uiElems.window:setVisible(true, true)
    end
...
function linesListGui.createLineTable()
    ...
    uiElems.lineTable:setColWidth(2,80)
...
function linesListGui.fillLineTable()
    ...
    for _, lineStats in pairs(uiState.allLinesCache) do
        ...
        local lblMaxStn = api.gui.comp.TextView.new(tostring(maxStn))

        local compLongWait
        if maxMissedTrain > 0 then
